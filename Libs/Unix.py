...
from Libs.Units import KILOBYTE
...
class Unix(HostBase):
    ...
    def getSystemInfo(self):
        ...
        for line in lines:
            ...
            if len(originalInfo) <= 2:
                continue
            syncSystemInfo["kernel_name"] = originalInfo[0]
            originalInfo.remove(originalInfo[0])
            syncSystemInfo["host_name"] = originalInfo[0]
            originalInfo.remove(originalInfo[0])
            import Libs.Linux
            import Libs.Solaris
            if isinstance(self, Libs.Linux.Linux):
                syncSystemInfo["os_type"] = originalInfo.pop()
                syncSystemInfo["hardware_platform"] = originalInfo.pop()
                syncSystemInfo["processor_type"] = originalInfo.pop()
                syncSystemInfo["hardware_name"] = originalInfo.pop()
                syncSystemInfo["kernel_version"] = "".join(originalInfo)
            elif isinstance(self, Libs.Solaris.Solaris):
                syncSystemInfo["os_type"] = 'Solaris'
                syncSystemInfo["kernel_version"] = originalInfo[0]
                syncSystemInfo["hardware_name"] = originalInfo[1]
                syncSystemInfo["processor_type"] = originalInfo[2]
                syncSystemInfo["hardware_platform"] = originalInfo[3]
            else:
                pass
    ...
    def getHostname(self, domainFlag=False):
        ...
        Args:
             domainFlag (bool)
        """
        infoHash = self.getSystemInfo()
        hostname = infoHash["host_name"]
        if domainFlag:
            domain = self.getDomainInfo()
            if not domain:
                raise UniAutosException('Could not find the FQDN for this host. Make sure it is'
    ...
    def which(self, program):
        ...
        Args:
             program (str)
        """
        response = self.run({"command":["sh", "-c", "which", program]})
        pattern = "no %s in | %s not found" % (program, program)
        if not response["stdout"] or re.search(pattern, response["stdout"], re.IGNORECASE):
            host = self.getHostname()
            msg = "%s was not found in %s's"% (program, host)
            raise FileNotFoundException(msg)
        lines = self.split(self.trim(response["stdout"]))
        for line in lines:
            if re.search(r'which', line):
                continue
            elif re.search(r'' + str(program) + '', line):
                return line
    ...
    def getDomainInfo(self):
        ...
        domain = None
        if self.doesPathExist({"path": resolvConfFile}):
            response = self.run({"command":["sh", "-c", "cat", resolvConfFile]})
            if response["rc"] != 0:
                raise CommandException("Get Domain Info Failed")
            lines = self.split(self.trim(response["stdout"]))
            for line in lines:
                matcher = re.match("^domain (\S+)$", line)
                if matcher:
                    domain = matcher.groups()[0]
                    break
        return domain
    ...
    def umount(self, mount):
        ...
        Args:
             mount (str)
        """
        response = self.run({"command":["sh", "-c", "umount", mount]})
        if response["rc"] != 0:
            if re.search(r'not mounted', response["stderr"]):
                return
            else:
                raise CommandException("Could not umount %s. %s"%(mount,
                                                                  response["stderr"]))
    ...
    def deleteDirectory(self, path):
        ...
        Args:
            path (str)
        """
        if self.doesPathExist({"path":path}) is not True:
            return
        if path == "/":
            raise InvalidParamException("Attempting to delete root is not allowed")
        response = self.run({"command": ["sh", "-c", "rm", "-rf", path]})
        if response["rc"] != 0:
            raise CommandException("Unable to delete %s" % (path))
    ...
    def createDirectory(self, path):
        ...
        if self.doesPathExist({"path": path}) is not True:
            return
        response = self.run({"command": ["sh", "-c", "mkdir", "p", path]})
        if response["rc"] != 0:
            raise CommandException("Unable to create given directory %s" % (path))

    def switchToDirectory(self, path):
        """
        Args:
            path (str)
        """
        response = self.run({"command": ["sh", "-c", "cd", path]})
        if response["rc"] != 0:
            raise CommandException("Unable to switch to given directory %s" % (path))
    ...
    def getMountPoints(self, device=None):
        ...
        Args:
            device (str|None)
        """
        response = self.run({"command": ["sh", "-c", "mount", "-v"]})
        deviceDict = {}
        if response["rc"] != 0:
            raise CommandException("Command execute Failed!")

        for line in self.split(response["stdout"]):
            line = self.trim(line)
            tmpMatch = re.match(r'^(\S+) on (.+?) type (\S+)', line)
            if tmpMatch:
                lineDevice = tmpMatch.group(1)
                lineMount = tmpMatch.group(2)
                fsType = tmpMatch.group(3)
                if device is not None and lineDevice != device:
                    continue
                if lineDevice == '/':
                    self.rootDisk = lineDevice
                deviceDict[lineDevice] = {"partition": lineDevice,
                                          "type": fsType}
                if "mount_points" not in deviceDict[lineDevice]:
                    deviceDict[lineDevice]["mount_points"]=[lineMount]
                else:
                    deviceDict[lineDevice]["mount_points"].append(lineMount)
        return deviceDict
    ...
    def getDateAndTime(self):
        ...
        """
        dateTime = {}
        response = self.run({"command":["sh", "-c", "date", "+%Y:%m:%d:%T"]})
        if response["rc"] != 0:
            raise CommandException('get host datetime failed')
        lines = self.split(response["stdout"])
        for line in lines:
            if re.match(r'^(\d+):(\d+):(\d+):(\d+):(\d+):(\d+)$', line):
                output = self.trim(line).split(":")
                dateTime = {"year":output[0],
                            "month":output[1],
                            "day":output[2],
                            "hour":output[3],
                            "minute":output[4],
    ...
    def createMount(self, device, mountPoint):
        """
        Args:
            device (str)
            mountPoint (str)
        ...
        response = self.run({"command":["sh", "-c", "mount", device, mountPoint]})
        if response["rc"] != 0:
            raise CommandException("Could not mount %s onto mount point %s \n Error:%s"%(device,
                                                                                         mountPoint,
                                                                                         response["stderr"]))
    ...
    def createFile(self, filePath, username=None, password=None):
        ...
        if username and password:
            userOptions = {"username": username, "password": password}
    ...
    def setFileAccessPermissions(self, user, permission, path):
        ...
        if response["rc"] != 0:
            raise CommandException("Problem copying file: %s \n Error: %s" % (path, response["stderr"]))

    def removeDirectory(self, path, newPath):
    ...
    def _createRandomDirectory(self, baseDir, template):
        ...
        if self.doesPathExist({"path": randomDir}) and var > 0:
            self.logger.info("The want to be create directory is exist, but template have some '%c 'or '%d', generate again.")
    ...
    def getCpuUsage(self):
        ...
        for line in cpuArray:
            if re.search(r'\s+CPU\s+', line):
                ...
                d = float(100 - float(a))
                tmpInfo["core%s" % (int(processorArray[coreNumberIndex]) + 1)] = "%.2f" % float(100 - float(processorArray[idleUsageIndex]))
    ...
    def scp(self, ip, remotePath, localPath, type, username='ibc_os_hs', password='Storage@21st', timeout=600):
        ...
        # 远端拷贝到本地
        if type == 'input':
            response = self.run({"command": ["sh","-c","scp", remoteFilePath, "'%s'"%localPath],
                                 "timeout": timeout, "waitstr": "yes\/no|assword"})
            if response['stdout']:
                if 'continue' in response['stdout']:
                    self.run({"command": ['yes'], "waitstr": "assword", "timeout": timeout, "input": [password, "[>#]"]})
                elif 'assword' in response['stdout']:
                    self.run({"command": [password], "timeout": timeout})
        # 本地拷贝到远端
        # 2018-03-13 wwx271515 适配output情况下需要输入yes或no场景
        if type == 'output':
            response = self.run({"command": ["sh","-c","scp", "'%s'"%localPath, remoteFilePath],
                                 "timeout": timeout, "waitstr": "yes\/no|assword"})
            if response['stdout']:
                if 'continue' in response['stdout']:
                    self.run({"command": ['yes'], "waitstr": "assword", "timeout": timeout, "input": [password, "[>#]"]})
