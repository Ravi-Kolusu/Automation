...
class DeviceBase(object):
    ...
    def addType(self, alias, classFullName):
        ...
        if not classFullName.startswith('UniAutos.Component.'):
            raise InvalidParamException('Alias: ' + alias + ', ClassFullName: ' + classFullName + ' is not a Component')
    ...
    def removeComponent(self, *args):
        ...
        try:
            ...
            for component in args:
                fullClassName = component.__module__ + '.' + component.__class__.__name__
                fullClassName = fullClassName.lower()
                self.logger.debug('Removing component: %s, Id: %s' % (str(component.properties),
                                                                      id(component)))
                component.zombify()
    ...
    def __compareCriteria(self, propertyValue, criteria):
        ...
        if isinstance(criteria, list):
            ...
            returnBool = True
            checkMsgs.append("Component %s has %s Class locks request waiting in the queue " % (fullName, inQueue))
