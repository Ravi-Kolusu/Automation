...
from UniAutos.Util.FtpClient import FtpClient
...
matcher = re.match('^\s*IPv4 Address.+: (\d+\.\d+\.\d+\.\d+)'
                   '|^\s*IPv4 地址.+: (\d+\.\d+\.\d+\.\d+)', line)
...
self.networkInfo['interface'][currentInterface]['ipv4_address'] = matcher.group(1) \
    if matcher.group(1) else matcher.group(2)
continue

matcher = re.match('^\s*.*IPv6 Address.+:\s+(.+)\(Preferred\)'
                   '|^\s*.*IPv6 地址.+:\s+(.+)\(首选\)', line)
if matcher and currentInterface:
    if currentInterface not in self.networkInfo['interface']:
        self.networkInfo['interface'][currentInterface] = {}
self.networkInfo['interface'][currentInterface]['ipv6_address'] = matcher.group(1) \
    if matcher.group(1) else matcher.group(2)
continue
...
if matcher and currentInterface:
    if currentInterface not in self.networkInfo['interface']:
        self.networkInfo['interface'][currentInterface] = {}
self.networkInfo['interface'][currentInterface]['netmask'] = matcher.group(1) \
    if matcher.group(1) else matcher.group(2)
continue
...
def addTargetPortal(self, ip, port="3260", chapUser=None, chapPassword=None,
...
cmd = ["cmd", "/c", "iscsicli", "LoginTarget", targetIqn, "T",
       targetPortal, port, "*", "*", "*", "2", headerDigest, dataDigest, "*", "*", "*"]
...
if targetNameMatch:
    sessionMaps[curSession]["target"] = targetNameMatch.group(1) \
        if targetNameMatch.group(1) else targetNameMatch.group(2)

continue

initiatorMatch = re.search(r'Initiator\s*:\s*(.+)|发起程序\s*:\s*(.+)', line)
if initiatorMatch:
    sessionMaps[curSession]["initiator"] = initiatorMatch.group(1) \
        if initiatorMatch.group(1) else initiatorMatch.group(2)
continue

scsiDeviceMatch = re.search(r'Initiator Scsi Device\s*:\s*(.+)|发起程序 SCSI 设备\s*:\s*(.+)', line)
if scsiDeviceMatch:
    sessionMaps[curSession]["scsi_device"] = scsiDeviceMatch.group(1) \
        if scsiDeviceMatch.group(1) else scsiDeviceMatch.group(2)
continue

scsiBusMatch = re.search(r'Initiator Bus\s*:\s*(.+)|发起程序总线\s*:\s*(.+)', line)
if scsiBusMatch:
    sessionMaps[curSession]["scsi_bus"] = scsiBusMatch.group(1) \
        if scsiBusMatch.group(1) else scsiBusMatch.group(2)
continue

scsiTargetIdMatch = re.search(r'Initiator Target Id\s*:\s*(.+)|发起程序目标 ID\s*:\s*(.+)', line)
if scsiTargetIdMatch:
    sessionMaps[curSession]["scsi_target_id"] = scsiTargetIdMatch.group(1) \
        if scsiTargetIdMatch.group(1) else scsiTargetIdMatch.group(2)
continue

lunMatch = re.search(r'Target Lun\s*:\s*0x(\S+) OS Lun\s*:\s*0x(\S+)'
                     r'|目标 Lun\s*:\s*0x(\S+) OS Lun\s*:\s*0x(\S+)', line)
...
if targetMatch and startLooking == 1:
    sessionMaps[sessionId]["portal"] = targetMatch.group(1) if targetMatch.group(
        1) else targetMatch.group(2)
break
return sessionMaps
...
if response["rc"] != 0 or \
        not self.doesPathExist(pathExitParams):
...
if match:
    disk = {"disk": match.group(1), "id": match.group(2), "status": match.group(3),
            "size": match.group(4), "size_company": match.group(5), "free": match.group(6),
...
if "mount" in params and params["mount"]:
    raise CommandException("The mount directory you have"
                           " specified is not empty: %s" % params["mount"])

elif "letter" in params and params["letter"]:
    raise CommandException("The letter you have specified "
                           "is already taken: %s" % params["letter"])
...
mount = self.createPartition({"lun": lunComponent, "disk": disk,
                              "mount": mount, "filesystem": filesystem,
                              "letter": letter, "unit_size": unitSize})
...
self.putFile(
    {'source_file': src_file,
     'destination_file': "C:/runSerial.py"}
)
...
if not (isinstance(ipValue, str)) or not (isinstance(kwargs["mask"], str)):
    raise InvalidParamException(
        "The value of ip or value of mask is invalid. ip: %s, mask: %s" % (ipValue, kwargs["mask"]))
...
# 执行命令
response = self.run({"command": ["cmd", "/c", "netsh", "interface", "ip", "set", "address", namecmd, cmd2,
                                 ipcmd, maskcmd, gatewaycmd, cmd3]})
# 判断命令是否执行成功
if response["rc"] != 0:
...
def getFileFromSftp(self, sftpIp, sftpUser, sftpPwd, sourPath, destPath="D:\\",
                    localPath=os.path.join(os.getcwd(), "SVP"), deleteLocal=False):
...
size_number = self.run_powershell(
    {'command': ['"(dir "%s" -rec -file | measure length -sum).sum/1%s"' % (dir_path, size_unit)]})
return float(size_number)