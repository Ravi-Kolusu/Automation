...
class SnmpV1(SnmpBase):
    ...
    SNMPv1
    """
    def __init__(self, hostIp, port=161, communityData=None, userDefineMibsDir=None, timeout=3, retries=5):
        """SNMP V1版本构造函数
    ...
    def get(self, *mibNodeNames):
        ...
        *mibNodeName (tuple) : 待操作的Mib节点名称列表。如：deviceId
        ...
        errorIndication, errorStatus, errorIndex, varBinds = self.cmdGen.getCmd(self.communityData, self.target, *oids)
    ...
    def getNext(self, *mibNodeNames):
        ...
        errorIndication, errorStatus, errorIndex, varBindTable = self.cmdGen.nextCmd(self.communityData,
                                                                                     self.target,
                                                                                     *oids)
    ...
    def getBulk(self, nonRepeaters, maxRepetitions, *mibNodeNames):
        ...
        for mibNodeName in mibNodeNames:
            oids.append(self.mibName2Oid(mibNodeName))
        errorIndication, errorStatus, errorIndex, varBindTable = self.cmdGen.bulkCmd(self.communityData,
                                                                                     self.target,
                                                                                     nonRepeaters,
                                                                                     maxRepetitions,
                                                                                     *oids)
    ...
    def set(self, *mibNodeNameValuePairs):
        ...
        errorIndication, errorStatus, errorIndex, varBinds = self.cmdGen.setCmd(self.communityData,
                                                                                self.target,
                                                                                *valuePairs)
    ...
    def snmpGet(self, index='0', *mibNodeNames):
        ...
        *mibNodeName (tuple) : 待操作的Mib节点名称列表。如：hwInfoFileSysID
        ...
        errorIndication, errorStatus, errorIndex, varBinds = self.cmdGen.getCmd(self.communityData,
                                                                                self.target,
                                                                                *oids)
        return self.parseResult(errorIndication, errorStatus, errorIndex, varBinds)
