...
import json
import codecs
from Libs.Time import repeat_timeout
...
from Libs.DeviceBase import DeviceBase
...
class HostBase(DeviceBase, Dispatcher):
    ...
    def discoverCommandInstance(cls, username, password, params):
        ...
        for k in ['username', 'password', 'newpassword', 'key', 'docker_ip', 'docker_user',
                  'docker_password', 'docker_port', 'port', 'max_session', 'debug_username',
                  'debug_password', 'ssh_private_key', 'ssh_public_key', 'heartbeatIp', "vrf_inner_flag"]:
            v = params.get(k, None)
            if v:
                temp[k] = v
        if "svp" in params:
    ...
    def run(self, params):
        ...
        """
        workingDir = ''
        if 'sessionType' not in params and ('UniAutos.Device.Host.Controller' in self.__class__.__module__ or
                                            'UniAutos.Device.Host.IPenclosure' in self.__class__.__module__) and 'wrapper' in params:
        ...
        if 'UniAutos.Device.Host.Controller' in self.__class__.__module__:
            if ("6.0." in productVersion and "D" in self.productModel) or self.command.protocol == "emustorssh":
        ...
        while validate_retry(result, retry):
            # 重试.
            self.logger.info('###[UniAutos Retry]:### [Host: %s] %s, Command: %s, Retry Count: %s'%(self.localIP,
                                                                                                    workingDir,
                                                                                                    " ".join(params["command"]),
                                                                                                    retry))
            cmdMsg = "[Host: %s] %s %s" % (self.localIP, workingDir, " ".join(params["command"]))
            ...
            retry += 1
            self.logger.info("###[UniAutos Retry]:### Retry interval, wait %sS, Retry command: %s" % (self.retry_interval, str(params["command"])))
            # 等待.
        ...
        def validateFunc(info):
            ...
            for ignore_code in self.wrapper_ignores.iterkeys():
                if method in self.wrapper_ignores[ignore_code]:
                    ...
                    if matcher:
                        self.logger.warn("###[UniAutos Ignore]:### Command Result Have Ignore Code: %s, Ignore this command error." % ignore_code)
            ...
            for line in lineRaw:
                ...
                matcher = re.search("\^(\n|\n\r|\r)?|Get wwn failed:sdId|Error:|Command failed|command not found|Try \'help\' for more information", line, re.IGNORECASE)
                if matcher:
                    return False
                return True
    ...
    def split(self, content):
        ...
        if not content:
            return []
        return re.split("\x0d?\x0a|\x0d", content)

    def writeCommandLog(self, params, logCommand=False):
        """
        Args:
            params (dict) :: params = {"start_time" : "",
                                       "command" : "",
                                       "execute_time" : ""}
        """
        msg = ["TIME_STAMP: %s\n" % params['start_time'],
        ...
        cmdExecFile = open(self.commandLogPath, 'a')
        cmdExecFile.writelines(msg)
        cmdExecFile.close()
        gCommandLogLock.release()
        wParams = {"log_id": Log.CommandLogId,
                   "command": params.get("command"),
                   "start_time_stamp": params.get("start_time_stamp"),
                   "sn": params.get("sn"),
                   "end_time_stamp": params.get("end_time_stamp")}
        try:
            if Log.CommandLogUri and logCommand:
                requests.post('http://%s/api/v1/command'%Log.CommandLogUri,
                              data=wParams,
                              timeout=2)
        except Exception:
            self.logger.trace("Write command information to database error. detail: %s"%(traceback.format_exc()))
    ...
    def isController(self):
        ...
        False- is not Windows
        """
        from Libs.ControllerBase import ControllerBase
    ...
    def runSQL(self, params):
        ...
        cmdMsg = "[Host: %s] %s %s %s" % (self.localIP, connectionParams["username"], connectionParams["dbUser"],
                                          " ".join(params["command"]))
        self.log.trace("Command: " + str(params["command"]))
        self.log.cmd(cmdMsg)
