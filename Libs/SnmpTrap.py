...
class SnmpTrap(SnmpBase):
    ...
    SNMP TRAP
    """
    def __init__(self, listenerIp, listenerPort=162, snmpVer='v3', userName=None, userDefineMibsDir=None,
                 communityData=('storage_public', 'storage_private'), authKey=None, privKey=None,
        ...
        config.addSocketTransport(self.__snmpEngine, udp.domainName,
                                  udp.UdpTransport().openServerMode((listenerIp, listenerPort)))

        if snmpVer == 'v3':
            config.addV3User(self.__snmpEngine,
                             userName=userName,
                             authKey=authKey,
                             privKey=privKey,
    ...
    def __receiveMessage(self, snmpEngine, stateReference, contextEngineId, contextName, varBinds, cbCtx):
        ...
        self.trapMessage.append(result)
        raise Exception('Invalid snmp trap message.')
